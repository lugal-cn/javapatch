			...
			<div class="main-container-inner">
				...
				<div class="sidebar" id="sidebar">
					...
					<!-- update-end-author:taoYan date:20180807 for: TASK #3052 【bug】论坛反馈 -- 3.7.8页面布局受到影响-->
						<li class="active">
							<a  href="javascript:addTabs({id:'home',title:'工作台',close: false,url: 'workbenchController.do?workbench'});">
				...
				<div class="main-content">
					<div class="page-content">
						<div class="row" style="height: 100%;">
							<div class="col-xs-12" style="width: 99%;padding-left:2px;padding-right: 2px;height: 100%;" id="tabs">
...
<input id="id" type="hidden" value="${user.id }">
	<table style="width: 550px" cellpadding="0" cellspacing="1" class="formtable">
		...
		jQuery(function($) {
			//$( "#tabs" ).tabs();
			addTabs({id:'home',title:'工作台',close: false,url: 'workbenchController.do?workbench'});
		...
		<audio src="/images/alarm.mp3" controls="controls" preload id="music" hidden></audio>
		<script type="text/javascript">
    	var yyalarmTime = ""; //语音告警时间最后一次
    	var tcalarmTime = "1970-1-1 00:00:00"; //弹窗告警时间最后一次
    	var switchOn=true;//这是弹窗开关
		//查询是否有语音播报告警和弹出告警信息
		var interval3 = setInterval(function() {
			 $.ajax({
				url : "workbenchController.do?getAlarmMsgByUserId",
				type : 'post',
				data : {yyalarmTime: yyalarmTime, tcalarmTime: tcalarmTime},
				cache : false,
				error:function(e){},
				success : function(data) {
					var d = $.parseJSON(data);
					if(d && d.obj) {
						if(d.obj.yyLstMore) { //语音   2反复播放
							control(); //2反复播放
							/* while(true) {
								control(); //2反复播放
							} */
						} else if(d.obj.yyLstOne) { //1播放一次，
							var yyLstOne=d.obj.yyLstOne;
							for(var i = 0; i < yyLstOne.length; i++){
							if(!yyalarmTime || (yyLstOne[i].alarm_time && yyLstOne[i].alarm_time > yyalarmTime)) {
								yyalarmTime = yyLstOne[i].alarm_time; //告警时间
							}
						 ...
						 if(d.obj.tcLst) { //弹窗 提示
							var tcLst = d.obj.tcLst;
						 if(tcLst[0].alarm_time > tcalarmTime){
							for (var i = 0; i < tcLst.length; i++) {
								if(!tcalarmTime || (tcLst[i].alarm_time && tcLst[i].alarm_time > tcalarmTime)) {
									tcalarmTime = tcLst[i].alarm_time; //告警时间
								}
								var computer_room_name = tcLst[i].computer_room_name;
								if(computer_room_name) {
									tip(computer_room_name+"有告警信息！");
								} else {
									tip("有告警信息！");
								}
							}
