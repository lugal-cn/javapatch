  ...
  /**
    ...
    */
    videojs.plugin('zoomrotate', function(settings){
        ...
        player = this.el();
        //video = this.el().getElementsByTagName('video')[0]; //写video标签和source用这行代码
        video = this.el().getElementsByTagName('object')[0]; //js生成的video用这行代码
        poster = this.el().getElementsByTagName('div')[1]; // div vjs-poster

        if (options.debug) console.log('zoomrotate: '+video.style);
        if (options.debug&&poster) console.log('zoomrotate: '+poster.style);
        if (options.debug&&options) console.log('zoomrotate: '+options.rotate);
        if (options.debug&&options) console.log('zoomrotate: '+options.zoom);
        ...
        if (options&&options.debug) console.log('zoomrotate: Register end');
    });
})();
