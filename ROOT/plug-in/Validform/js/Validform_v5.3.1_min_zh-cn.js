(function($, win, undef) {
	...
	Validform.util = {
		...
		ajax_uBit_check:function(fieldVlaue,param,param1,cabineNo){
			   //获取编辑页面的数据主键
					var obid = $("input[id='id']").val();
					
					
					$.ajaxSetup({ async: false});//同步ajax 
					var check_flag="";
					$.ajax({
						url : 'tbCrEquipmentController.do?uBitCheck',
						async:false,
						data : {
							fieldVlaue: fieldVlaue,
							rowObid   : obid,
			...
			ajax_ip_check:function(fieldVlaue){
					   //获取编辑页面的数据主键
					var obid = $("input[id='id']").val();
					var param = $("input[id='computerRoomId']").val();
					
					$.ajaxSetup({ async: false});//同步ajax 
					var check_flag="";
					$.ajax({
						url : 'tbCrEquipmentController.do?uIpCheck',
						async:false,
						data : {
							fieldVlaue: fieldVlaue,
							rowObid   : obid,
			...
			ajax_cabineNo_check:function(fieldVlaue){
				var check_flag="";
				var obid = $("input[id='id']").val();
				var computerRoomId = $("input[id='computerRoomId']").val();
				var cabineNoCur = fieldVlaue;
			  	if(!cabineNoCur){
			  		return;
			  	}
			  	//和页面上输入的验证  1获取页面上所有的机柜编码
			  	var cabineNos = $("#add_jformGraphreportItem_table input[name$='cabineNo']");
			  	if(cabineNos) {
			  		var cabineNo = "";
			  		var n = 0;
			  		for(var i = 0; i < cabineNos.length; i++){
			 			cabineNo = cabineNos[i].value;
						if(cabineNo == cabineNoCur) { //相同数量
							n++;
						} else { //不提示
						}
						if(n >= 2) {
							check_flag = "机柜编号重复！+false";
							return check_flag;
						}
			  		}
			  	}
				$.ajaxSetup({ async: false});//同步ajax 
				$.ajax({
					url : 'tbCrCabineController.do?cabineNoCheck',
					async:false,
					data : {
						fieldVlaue: fieldVlaue,
		...
		check : function(curform, brothers, subpost, bool) {
		    ...
		    //}
			//U位信息不在数据库里存在
			var  validUbit=$(this).attr("validUbit");
			//var tipType=settings.tiptype;
			if(validUbit!=null){
		       var params=validUbit.split(",");
		       var  ajaxResultValue=Validform.util.ajax_uBit_check($(this).val(),$("input[name='"+params[0]+"']").val(),params[0],$("input[name='cabineNo']").val());
		       var resultParams= new Array(); //定义一数组
                resultParams=ajaxResultValue.split("+"); //字符分割     
			   if (resultParams[1] == "false" && tipType == 1) {
                    msgobj.find(".Validform_info").html(resultParams[0]);
			...
			//IP在同一个机房不能重复
			var  validEquipmentIp=$(this).attr("validEquipmentIp");
			//var tipType=settings.tiptype;
			if(validEquipmentIp!=null){
		       var  ajaxResultValue=Validform.util.ajax_ip_check($(this).val());
		       var resultParams= new Array(); //定义一数组
                resultParams=ajaxResultValue.split("+"); //字符分割     
			   if (resultParams[1] == "false" && tipType == 1) {
                    msgobj.find(".Validform_info").html(resultParams[0]);
		...
		submitForm : function(settings, flg, url, ajaxPost, sync) {
			...
			if(tipType==null||tipType!=1){
				...
				//----------------------------0826
				 curform.find("[validUbit]").each(function(obj){
				   var validUbit=$(this).attr("validUbit");
					if(validUbit!=null&&$(this).val()!=""){
				       var params=validUbit.split(",");
				       var ajaxResultValue=Validform.util.ajax_uBit_check($(this).val(),$("input[name='"+params[0]+"']").val(),params[0],$("input[name='cabineNo']").val());
					   var resultParams= new Array(); //定义一数组
		                resultParams=ajaxResultValue.split("+"); //字符分割     
				...
				//----------------------------0826
				 curform.find("[validEquipmentIp]").each(function(obj){
				   var validEquipmentIp=$(this).attr("validEquipmentIp");
					if(validEquipmentIp!=null&&$(this).val()!=""){
				       var ajaxResultValue=Validform.util.ajax_ip_check($(this).val());
					   var resultParams= new Array(); //定义一数组
		                resultParams=ajaxResultValue.split("+"); //字符分割     
					  if(resultParams[1]=="false"){
				...
				//----------------------------1011
				 curform.find("[validCabineNo]").each(function(obj){
				   var validCabineNo=$(this).attr("validCabineNo");
					if(validCabineNo!=null&&$(this).val()!=""){
				       var ajaxResultValue=Validform.util.ajax_cabineNo_check($(this).val());
					   var resultParams= new Array(); //定义一数组
		                resultParams=ajaxResultValue.split("+"); //字符分割     
					  if(resultParams[1]=="false"){
						  if(typeof tipType == "function"){
